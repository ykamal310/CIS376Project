...
def change_equipment_status(user, equipment_id, current_status):
    ...
    changed = update_equipment_status(equipment_id, new_status)

    if not changed:
        return False, "Equipment could not be updated."

    return True, f"Equipment is now {new_status}."


def cancel_admin_reservation(user, reservation_id):
    if not is_admin(user):
        return False, "Administrator access is required."
    ...
    if reservation["status"] != "Scheduled":
        return False, "Only scheduled reservations can be cancelled."

    changed = mark_reservation_cancelled(reservation_id)
    ...
    update_used_minutes(reservation["user_id"], -minutes)

    return True, "Reservation cancelled successfully."


def change_user_role(admin, user_id, current_role):
    ...
    changed = update_user_role(user_id, new_role)
...
def add_student_time(admin, user_id, minutes):
    if not is_admin(admin):
        return False, "Administrator access is required."
    ...
    changed = extend_time_budget(user_id, minutes)

    if not changed:
        return False, "This user does not have a student time budget."

    return True, f"Added {minutes} minutes to the student's weekly budget."
...
def modify_admin_reservation(
    ...
    new_start_datetime = datetime.combine(new_date, start.time())
    ...
    old_start = datetime.strptime(reservation["start_time"], "%H:%M")

    old_end = datetime.strptime(reservation["end_time"], "%H:%M")

    old_minutes = int((old_end - old_start).total_seconds() / 60)
    ...
    if difference > 0:
        budget = get_time_budget(reservation["user_id"])

        if budget:
            remaining = budget["weekly_minutes"] - budget["used_minutes"]

            if difference > remaining:
                return False, "Student does not have enough lab time."
    ...
    if difference != 0:
        update_used_minutes(reservation["user_id"], difference)
